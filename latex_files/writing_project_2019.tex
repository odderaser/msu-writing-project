...
\[P(i > j) = \left\{
    ...
    \frac{\theta\lambda_i}{\theta\lambda_i + \lambda_j} & : \text{if } i \text{ has the advantage}\\
		          \frac{\lambda_i}{\lambda_i + \theta\lambda_j} & : \text{if } j \text{ has the advantage}\\
    \frac{\lambda_i}{\lambda_i + \lambda_j} & : \text{if neither } i \text{ or } j \text{ has an advantage}\\
			    \end{array}
			    \right.
			    \]

Now, it is important to note that in the case of this project, the advantages under consideration are static. That is, if a character $i$ has a hitbox advantage against character $j$, this will always be the case in each match played against each other. Therefore, exactly one of $w_{ij}^{+}, w_{ij}^{-},$ or $w_{ij}^{*}$ will be greater than zero for some combination of $i$ and $j$. In this case, $w_{ij}^{*}$ represents non-advantaged matches where the hitbox differential is such that there is no defined advantage. Using this new probabability statement in the same way the previous two likelihood functions have been derived, we see that we have the following:

\begin{equation}
\begin{split}
L(\lambda_i, \theta) = \underset{i \neq j}{\prod_{i=1}^{m} \prod{j=1}^{m}} \bigg(\frac{\theta\lambda_i}{\theta\lambda_i + \lambda_j}\bigg)^{w_{ij}^{+}}\bigg(\frac{\lambda_i}{\lambda_i + \theta\lambda_j}\bigg)^{w_{ij}^{-}}\bigg(\frac{\lambda_i}{\lambda_i + \lambda_j}\bigg)^{w_{ij}^{*}} \\
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
	\text{ln}L(\lambda_i, \theta) & = \underset{i \neq j}{\sum_{i=1}^{m} \sum_{j=1}^{m}} \text{ln}\bigg(\bigg(\frac{\theta\lambda_i}{\theta\lambda_i + \lambda_j}\bigg)^{w_{ij}^{+}}\bigg(\frac{\lambda_i}{\lambda_i + \theta\lambda_j}\bigg)^{w_{ij}^{-}}\bigg(\frac{\lambda_i}{\lambda_i + \lambda_j}\bigg)^{w_{ij}^{*}}\bigg) \\
	& = \underset{i \neq j}{\sum_{i=1}^{m} \sum_{j=1}^{m}} w_{ij}^{+}\text{ln}(\theta\lambda_i) - w_{ij}^{-}\text{ln}(\theta\lambda_i + \lambda_j) + w_{ij}^{-}\text{ln}(\lambda_i) - w_{ij}^{-}\text{ln}(\lambda_i + \theta\lambda_j) + w_{ij}^{*}\text{ln}(\lambda_i) - w_{ij}^{*}\text{ln}(\lambda_i + \lambda_j) \\
	& = n^{+}\text{ln}(\theta) + \sum_{i=1}^{m} w_{i} \text{ln}(\lambda_i) - \sum_{j=1}^{m} (w_{ij}^{+} \text{ln} (\theta\lambda_i + \lambda_j) + w_{ij}^{-}\text{ln}(\lambda_i + \theta\lambda_j) + w_{ij}^{*}\text{ln}(\lambda_i + \lambda_j) \\
\end{split}
\end{equation}
...
The original algorithm for estimating probabilities of paired comparisons was developed before Thurstone formulated a model in full. Zermelo in 1929 developed and proved an algorithm which converged to a unique set of parameter estimates given certain conditions were met \cite{hunter2004mm}. These conditions are discussed more fully in section 3 as an analysis of the comparison graph. The algorithm described by Zermelo is described below:

\RestyleAlgo{boxruled}
\LinesNumbered
\begin{algorithm}[ht]
	\caption{Zermelo's Algorithm\label{alg}}
	Randomly initialize a vector s.t. $sum_{i=1}^{m}\lambda_i = 1$ and $lambda_i \geq 0$ \\
	While not convergence criteria not met \\
	    For $i$ in $1:m$ \\
		$\lambda_i' = w_i \bigg(\sum_{i \neq j}\frac{n_{ij}}{\lambda_i + \lambda_j}\bigg)^{-1}$ \\
	    Renormalize all $\lambda_i$ \\
	Return $\lambda_i$ \\
\end{algorithm}
